...
[`src/test/kotlin/com/example/jetbrains/integration/BonusTests.kt`](src/test/kotlin/com/example/jetbrains/integration/BonusTests.kt) contains additional IDEA Ultimate Settings UI checks. Keep them focused on stable Settings interactions:
- **Editor** > **General** > **Auto Import** changes **Insert imports on paste** from `Always` to `Ask` and verifies the selected combo box value.
- **Editor** > **Font** changes the font size and verifies the field value.
